...
void loop() {
    ...
    float turbidityValue_T2 = calculateTurbidity(rawTurbidity);

    int rawPH = analogRead(phPin);
    float phvalue_T2 = calculatePH(rawPH, 5.0, 7.0, 2.5);

    int rawLevel = analogRead(levelPin);
    float levelvalue_T2 = calculateLevel(rawLevel);

    int rawTDS = analogRead(tdsPin);
    float tdsValue_T2 = calculateTDS(rawTDS);

    // Display Sensor Data on LCD
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Turb: "); lcd.print(turbidityValue_T2); lcd.print(" NTU");
    lcd.setCursor(0, 1);
    lcd.print("pH: "); lcd.print(phvalue_T2);

    delay(3000); // Give time to view readings

    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Level: "); lcd.print(levelvalue_T2); lcd.print("%");
    lcd.setCursor(0, 1);
    lcd.print("TDS: "); lcd.print(tdsValue_T2); lcd.print(" ppm");
    ...
    String message = "TANK 2 - Turb: " + String(turbidityValue_T2) +
                     ", pH: " + String(phvalue_T2) +
                     ", Level: " + String(levelvalue_T2) +
                     ", TDS: " + String(tdsValue_T2);
